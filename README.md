...
`askance`. It is the same derivation `nix run` builds, so the host needs no
checkout of this repository at all.

Three steps, in a host's own flake-based configuration. Askance is a private
repository, so the token comes first: without it, nothing below gets as far as
building.

### 1. Give nix a GitHub token

nix fetches the input during evaluation, and a fine-grained personal access
token with read-only **Contents** on this one repository is all that takes.
Check the token before wiring anything up:

```console
$ curl -s -o /dev/null -w '%{http_code}\n' \
    -H "Authorization: Bearer ghp_..." \
    https://api.github.com/repos/tobico/askance/commits/HEAD
200
```

Worth doing, because every way this can go wrong looks the same from nix. A
token that is missing, a token that does not name the repository, and a
repository that genuinely is not there all come back as `unable to download ...
HTTP error 404` — which reads like the last of the three when it is almost
always one of the first two.

Two identities fetch the input, so the token goes in two places. Your own user
locks
the input:

```console
$ mkdir -p ~/.config/nix
$ install -m 600 /dev/null ~/.config/nix/nix.conf
$ printf 'access-tokens = github.com=%s\n' 'ghp_...' > ~/.config/nix/nix.conf
```

and root evaluates the configuration under `nixos-rebuild`. Root's copy cannot
live in the configuration itself — that is usually a public repository — so it
goes in a root-owned file that `nix.conf` includes:

```console
$ sudo install -m 600 /dev/null /etc/nix/access-tokens.conf
$ sudo tee /etc/nix/access-tokens.conf <<<'access-tokens = github.com=ghp_...'
```

with the configuration saying only where to find it:
...
`!include` is the tolerant form — a host without that file still runs every
other nix command, and only fetching a private input fails.

That include is generated by the rebuild, which means it is not in force during
the rebuild that first installs it. Lock as your own user before switching and
the source is already in the store when root evaluates; if root reaches for the
network anyway, `--option access-tokens "github.com=ghp_..."` on that one
`nixos-rebuild` breaks the cycle. Every rebuild after it reads the file.
...
New commits reach the host the way any other input does: `nix flake update
askance` in the host configuration, then rebuild.
